...
const EnvArg = option({
	...
	long: "env",
	short: "e",
	description: "environment config to use during generation, config will be loaded from `config.<env>.<yaml|yml|json>`",
});
type T = typeof EnvArg;
interface EnvArg extends T {}
function env(): EnvArg {
	return EnvArg;
}
